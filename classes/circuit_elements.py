...
class Leg:
    ...
    def current(self, phase, threaded_flux=0) -> np.ndarray:
        ...
        self.assign_vars(closure_phase=threaded_flux)
        self.energy()
        self.current_data = np.concatenate((self.current_data, self.E.reshape(-1,1)), axis=1)
        self.stability()
        self.current_data = np.concatenate((self.current_data, self.stability_bool.reshape(-1,1)), axis=1)
        return self.current_data #[[phi_T], [I], [chi], [phi_ind], [E], [stability_bool]]

    def assign_vars(self, closure_component='JJ', closure_phase=0) -> None:
...
class LinRhombus:
    ...
    def current(self, phase, threaded_flux=np.pi):
        ...
        self.current_data = sum_matching_rows(self.leg1.current_data, self.leg2.current_data)
        self.assign_vars()

    def assign_vars(self):
        self.phi_T = self.current_data[:,0]/2 #NEED TO DIVIDE BY TWO HERE BECAUSE THE SUM MATCHING ROWS ALSO DOUBLES THE PHASE!
        self.I = self.current_data[:,1]
        self.chi1 = self.leg1.chi
        self.chi2 = self.leg2.chi 
        self.phi_ind1 = self.leg1.phi_ind
        self.phi_ind2 = self.leg2.phi_ind
        self.E = self.current_data[:,-2]
        self.current_data[:,-1] = np.where(self.current_data[:,-1]==2, 1, 0) # Only output 'stable' if both chi1 and chi2 have stable phases
        self.stability_bool = self.current_data[:,-1]
        self.stability_colormap = np.where(self.stability_bool==1, 'b', 'r')
